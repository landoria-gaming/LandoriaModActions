...
- `build-snapshot`: builds with the mod's HarmonyValidator and calls the shared PackageThunderstore target. Reference versions and source run are recorded in build metadata.
...
jobs:
  snapshot:
    permissions:
      contents: write
    uses: landoria-gaming/LandoriaModActions/.github/workflows/snapshot.yml@main
    with:
...
The shared `build/Thunderstore.targets` file owns PackageThunderstore. Workflows
inject it into MSBuild, so cloned mod projects have no LandoriaModActions dependency.
Files are at repository root, DLL output is bin/Release/<mod-name>.dll. No suffix
